...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (subcommand === "enable") {
      ...
      const serverSizeMultiplier =
        AdvancedAntiRaid.getServerSizeMultiplier(memberCount);
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription(
          `**Server Size Tier:** ${serverSizeTier}\n**Members:** ${memberCount.toLocaleString()}`
        )
        .addFields(
          {
            name: "Status",
        ...
        .setColor(0x0099ff)
        .setFooter({
          text: "Thresholds automatically scale with server size to prevent false positives",
        })
        .setTimestamp();
