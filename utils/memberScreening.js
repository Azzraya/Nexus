...
class MemberScreening {
  ...
  async screenMember(member, guild) {
    ...
    if (config.check_threat_intel) {
      ...
      const threatCheck = await ThreatIntelligence.checkThreat(
        member.user.id,
        guild.id
      );
    ...
    if (riskScore >= config.auto_ban_threshold) {
      ...
      riskScore >= config.quarantine_threshold &&
  ...
  async executeScreeningAction(member, screenResult, config) {
    try {
      ...
      switch (screenResult.action) {
        ...
        case "kick":
          await member.kick(
            `Member Screening: ${screenResult.reason} (Risk: ${screenResult.riskScore}%)`
          );
          logger.info(
            `[MemberScreening] Kicked ${member.user.tag} in ${guild.name}: ${screenResult.reason}`
          );
          break;
        ...
        case "quarantine":
          ...
          if (role) {
            ...
            logger.info(
              `[MemberScreening] Quarantined ${member.user.tag} in ${guild.name}: ${screenResult.reason}`
            );
          }
          break;
        ...
        case "alert":
          ...
          logger.info(
            `[MemberScreening] Alert for ${member.user.tag} in ${guild.name}: ${screenResult.reason}`
          );
          break;
      }
  ...
  async sendAlert(member, screenResult, config) {
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        ...
        {
          name: "Risk Score",
          value: `${screenResult.riskScore}%`,
          inline: true,
        },
        {
          name: "Flags",
          value: screenResult.flags.join("\n") || "None",
          inline: false,
        },
        {
  ...
  async logScreening(member, screenResult, config) {
    ...
    const embed = new EmbedBuilder()
      ...
      .setColor(
        screenResult.action === "ban"
          ? 0xff0000
          : screenResult.action === "kick"
            ? 0xff6600
            : 0xffaa00
      )
      .addFields(
  ...
  async getScreeningStats(guildId, days = 30) {
    ...
    const stats = {
      ...
      avgRiskScore:
        logs.length > 0
          ? Math.round(
              logs.reduce((sum, l) => sum + l.risk_score, 0) / logs.length
            )
          : 0,
      topFlags: {},
    };
